...
import {
  getConfig
} from '../../../src/commands/check/get-config';
...
describe('get-config tests', () => {
  ...
  it('defaults config file name if not specified', () => {
    ...
    expect(mockResolve).toBeCalledWith('precloud.config.json');
    expect(mockReadFileSync).toBeCalled();
    expect(mockReadFileSync).toBeCalledWith('precloud.config.json');
  ...
  it('logs error and defaults to empty object if the config file cannot be parsed as JSON', () => {
    ...
    expect(mockLogError).toBeCalledWith('Invalid config file! The contents of precloud.config.json could not be parsed as JSON.  Correct any syntax issues and try again.');
    expect(config).toEqual({});
  });
