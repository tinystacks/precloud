...
async function parseTfResource (diff: TfDiff, tfPlan: Json, config: CheckOptions): Promise<Json> {
  const {
    terraformParsers = []
  } = config;
...
async function parseTerraformDiff (planFile: string, config: CheckOptions): Promise<ResourceDiffRecord[]> {
