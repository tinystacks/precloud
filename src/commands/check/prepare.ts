...
async function prepareCdk (config: CheckOptions): Promise<ResourceDiffRecord[]> {
...
async function prepareTf (config: CheckOptions): Promise<ResourceDiffRecord[]> {
...
async function prepareForCheck (config: CheckOptions): Promise<ResourceDiffRecord[]> {
  createTmpDirectory();
  const { format } = config;
  switch (format) {
